 ...
 */
	...
	$.jstree.core.prototype = {
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			if(!node) { node = { "text" : this.get_string('Newss node') }; }
			if(typeof node === "string") { node = { "text" : node }; }
			if(node.text === undefined) { node.text = this.get_string('Newss node'); }
			var tmp, dpc, i, j;
 ...
 */
	...
	$.jstree.defaults.unique = {
		...
		case_sensitive : false,
		/**
		 * A callback executed in the instance's scope when a new node is created and the name is already taken, the two arguments are the conflicting name and the counter. The default will produce results like `Newss node (2)`.
	...
	$.jstree.plugins.unique = function (options, parent) {
		...
		this.create_node = function (par, node, pos, callback, is_loaded) {
			if(!node || node.text === undefined) {
				...
				n = tmp = this.get_string('Newss node');
				dpc = [];
				for(i = 0, j = par.children.length; i < j; i++) {
